...
function VideoCard({thumbnail,title,creatorName,reactionCount,creatorPic,liked,mediaUrl,post}) {
  
  return (
    <Link to={`/videoplayer/${mediaUrl.substring(mediaUrl.lastIndexOf("/")+1)}`} state={post}>
      <div className='flex flex-col gap-4 w-[15rem] rounded-[2rem] border-2 bg-[#15141B] border-purple-500 shadow-[rgba(0,_0,_0,_0.24)_0px_3px_8px] duration-200 hover:z-20 hover:scale-110 cursor-pointer'>
        ...
        <div className='flex gap-2 px-2 py-2'>
            ...
            <div className='flex flex-col '>
                <p className='text-white'>{String(title).length>20?(String(title).substring(0,20)+"..."):title}</p>
                <div className='flex justify-between text-[#7F7D8D] flex-col '>
                    <p className=''>{String(creatorName).length>20?(String(creatorName).substring(0,20)+"..."):creatorName}</p>
